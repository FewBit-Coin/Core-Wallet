...
void static FewBitMiner(const CChainParams& chainparams)
{
    LogPrintf("FewBitMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fewbit-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("FewBitMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("FewBitMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("FewBitMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("FewBitMiner -- terminated\n");
        throw;
    } catch (const std::runtime_error& e) {
        LogPrintf("FewBitMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateFewBits(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++) {
        minerThreads->create_thread(boost::bind(&FewBitMiner, boost::cref(chainparams)));
    }
    return (numCores);
}
