...
const QString BITCOIN_IPC_PREFIX("fewbit:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/fewbit-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/fewbit-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/fewbit-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(interfaces::Node& node, int argc, char* argv[])
{
    for (int i = 1; i < argc; i++) {
        ...
        // If the fewbit: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // fewbit: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click fewbit: links
    ...
    if (startLocalServer) {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start fewbit: click-to-pay handler"));
        } else {
...
//
// OSX-specific way of handling fewbit: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in fewbit: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("fewbit://", Qt::CaseInsensitive)) {
        Q_EMIT message(tr("URI handling"), tr("'fewbit://' is not a valid URI. Use 'fewbit:' instead."),
            CClientUIInterface::MSG_ERROR);
    } else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // fewbit: URI
    {
        ...
        } else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient)) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid FewBit address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom fewbit addresses are not supported
        ...
        // FewBit amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
