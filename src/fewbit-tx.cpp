...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = strprintf("%s fewbit-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                               "Usage:\n"
                               "  fewbit-tx [options] <hex-tx> [commands]  Update hex-encoded fewbit transaction\n" +
                               "  fewbit-tx [options] -create [commands]   Create hex-encoded fewbit transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded fewbit transaction
