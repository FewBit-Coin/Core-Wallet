...
const char* const BITCOIN_CONF_FILENAME = "fewbit.conf";
const char* const BITCOIN_PID_FILENAME = "fewbitd.pid";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "fewbitd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FewBitCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FewBitCore
    // Mac: ~/Library/Application Support/FewBitCore
    // Unix: ~/.fewbitcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FewBitCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FewBitCore";
#else
    // Unix
    return pathRet / ".fewbitcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty fewbit.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure FewBit Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("FewBit Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The FewBit Core developers";
    }
