...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 FEWBIT irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on fewbitcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
        ...
        "Vote on a governance object by smartnode configured in fewbit.conf\n"
        "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("fewbit.conf", statusObj);
...
#endif // ENABLE_WALLET
        "  vote-conf          - Vote on a governance object by smartnode configured in fewbit.conf\n"
#ifdef ENABLE_WALLET
...
static const CRPCCommand commands[] =
    {
        ...
        //  --------------------- ------------------------  -----------------------  ----------
        /* FewBit features */
        {"fewbit", "getgovernanceinfo", &getgovernanceinfo, {}},
        {"fewbit", "getsuperblockbudget", &getsuperblockbudget, {"index"}},
        {"fewbit", "gobject", &gobject, {}},
        {"fewbit", "voteraw", &voteraw, {"tx_hash", "tx_index", "gov_hash", "signal", "outcome", "time", "sig"}},
