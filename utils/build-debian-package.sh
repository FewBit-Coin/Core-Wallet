...
PACKAGE_VERSION=$($SRC_PATH/src/fewbitd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/fewbitd $DEB_BIN
cp $SRC_PATH/src/fewbit-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/fewbit-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/fewbitd.1 $DEB_MAN
cp $SRC_DOC/man/fewbit-cli.1 $DEB_MAN
cp $SRC_DOC/man/fewbit-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/fewbitd.bash-completion $DEB_CMP/fewbitd
cp $SRC_PATH/contrib/fewbit-cli.bash-completion $DEB_CMP/fewbit-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/fewbitd.1
gzip --best -n $DEB_MAN/fewbit-cli.1
gzip --best -n $DEB_MAN/fewbit-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/fewbitd $DEB_BIN/fewbit-cli
