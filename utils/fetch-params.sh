...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/FewBitParams"
else
    PARAMS_DIR="$HOME/.fewbit-params"
fi
...
DOWNLOAD_URL="https://github.com/FewBit-Coin/Core-Wallet/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with FBIT_DISABLE_SOMETHING=1
FBIT_DISABLE_WGET="${FBIT_DISABLE_WGET:-}"
#FBIT_DISABLE_IPFS="${FBIT_DISABLE_IPFS:-}"
FBIT_DISABLE_CURL="${FBIT_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$FBIT_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$FBIT_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$FBIT_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the FewBit parameters!
...
main() {
    ...
    cat <<EOF
FBIT - fetch-params.sh

This script will fetch the FewBit SNARK parameters and verify their
...
EOF
    ...
    if ! [ -d "$PARAMS_DIR" ]; then
        ...
        cat >>"$README_PATH" <<EOF
This directory stores common FewBit SNARK parameters. Note that it is
