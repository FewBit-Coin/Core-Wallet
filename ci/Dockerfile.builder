...
# fewbit_hash
RUN git clone https://github.com/FewBit-Coin/Core-Wallet_hash
RUN cd fewbit_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} fewbit
RUN useradd -u ${USER_ID} -g fewbit -s /bin/bash -m -d /fewbit fewbit
...
RUN mkdir /fewbit-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /fewbit-src && \
